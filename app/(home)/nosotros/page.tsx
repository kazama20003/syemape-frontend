...
export default function AboutPage() {
  return (
    <main className="bg-[#f7f5f1] text-dark">
      <section className="flex min-h-[34rem] items-end bg-[#263e42] px-[var(--spacing-gutter)] pb-12 pt-28 text-white md:min-h-[43rem] md:pb-16">
        <div className="mx-auto w-full max-w-[120rem]">
          <p className="eyebrow">Quiénes somos</p>
          <h1 className="mt-5 max-w-[12ch] text-[clamp(3rem,6.5vw,7rem)] leading-[0.91]">Experiencia que acompaña cada ruta.</h1>
          <p className="mt-6 max-w-2xl text-sm leading-relaxed text-white/85">Somos una empresa arequipeña especializada en transporte y servicios operativos para los sectores minero, construcción e industria.</p>
        </div>
      </section>
      ...
      <section className="px-[var(--spacing-gutter)] py-20 md:py-28">
        <div className="mx-auto grid max-w-[120rem] gap-12 lg:grid-cols-[minmax(13rem,0.44fr)_1fr]">
          <p className="eyebrow">Quiénes somos</p>
          <div className="grid gap-10 lg:grid-cols-[1fr_minmax(17rem,0.8fr)] lg:items-start">
            <div>
              <h2 className="text-[length:var(--text-fluid-h2)]">Una empresa arequipeña que conoce la operación.</h2>
              <p className="mt-8 max-w-2xl text-base leading-relaxed">Somos S&amp;E MAPE E.I.R.L., una empresa arequipeña con más de 4 años de experiencia en el sector de transporte y servicios especializados. Nos especializamos en brindar soluciones integrales en alquiler de vehículos livianos y pesados, supervisión y escolta en ruta, y transporte terrestre de mercancías, atendiendo principalmente a los sectores minero, de construcción e industrial a nivel nacional.</p>
              <p className="mt-5 max-w-2xl text-base leading-relaxed text-dark/75">Nos distinguimos por nuestro compromiso con la seguridad, la eficiencia operativa y el cumplimiento de los más altos estándares de calidad, adaptándonos a los requerimientos específicos de cada cliente y contribuyendo al desarrollo seguro y oportuno de sus operaciones.</p>
            </div>
            <div className="bg-[#41577e] p-6 text-white md:p-8"><p className="u-text-small-caps text-white/65">Nuestro enfoque</p><p className="mt-8 text-2xl leading-tight">Seguridad, eficiencia operativa y calidad en cada decisión.</p><div className="mt-16 border-t border-white/35 pt-4 text-sm text-white/80">Contribuimos al desarrollo seguro y oportuno de las operaciones de nuestros clientes.</div></div>
          </div>
        </div>
      </section>

      <section className="bg-brand px-[var(--spacing-gutter)] py-16 text-white md:py-24">
        <div className="mx-auto max-w-[120rem] text-[clamp(2.25rem,5vw,5.4rem)] leading-[0.98]">No solo movemos carga. Sostenemos la continuidad de cada operación.</div>
      </section>
      ...
      <section className="px-[var(--spacing-gutter)] py-20 md:py-28">
        <div className="mx-auto grid max-w-[120rem] gap-12 lg:grid-cols-[minmax(13rem,0.44fr)_1fr]">
          <div><p className="eyebrow">Propuesta de valor</p><h2 className="mt-7 text-[length:var(--text-fluid-h2)]">Respaldo para operar con confianza.</h2><p className="mt-6 max-w-xs text-sm leading-relaxed text-dark/70">Un servicio basado en seguridad, confiabilidad y continuidad operativa.</p></div>
          <div className="border-t border-dark/35">
            {values.map(([title, description, Icon], index) => <article className="grid gap-4 border-b border-dark/35 py-6 md:grid-cols-[2rem_minmax(10rem,0.6fr)_1fr] md:items-start" key={title}><Icon className="h-6 w-6 text-brand" strokeWidth={1.4} /><h3 className="text-xl">{String(index + 1).padStart(2, "0")} · {title}</h3><p className="text-sm leading-relaxed text-dark/75">{description}</p></article>)}
          </div>
        </div>
      </section>
